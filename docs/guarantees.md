...
- Each surface binds its own pre-handler hook: `Http.layer({ before }, ...)`, `ActionMcp.layerHttp`/`layerStdio`, `ActionToolkit.make`, `ActionCli.command`/`group`. A bound hook runs once per invocation, before the selected handler and outside its span, for every action that surface serves. There is no per-action opt-out and no group that escapes it; only calling `build` directly does, and that is not dispatch.
...
- Ordering differs by surface, because each hooks into its own transport: HTTP runs the hook **before** the request payload is decoded, so an unauthorized caller learns nothing about the input schema. MCP, the Toolkit and the CLI run it after the native transport has decoded the arguments, since decoding is the transport's own first step there.
...
- `access` is contract metadata, required on every action and kept as a literal type. Nothing in the library reads it: adapters ignore it and the catalog omits it. It exists so a hook can switch on what an action does instead of on its name.
...
| Pre-handler hook failure    | the same, using the hook's error schema, before the payload is decoded; the handler never runs        | the same `isError` result, after arguments are decoded; the handler never runs                         |
...
- Every HTTP action response with a status of 400 or more carries `cache-control: no-store`. It is `ActionHttp`'s own router middleware, so it covers the action routes and nothing else: an `ActionMcp.layerHttp` endpoint, the router's 404 for an unmatched path, and any other route the host mounts answer without it. Successful responses are left to the host. An error response has no other added headers: a hook refusal and a handler error are plain declared errors, and challenge headers such as `WWW-Authenticate` belong to the admission middleware that answers before the router reaches these routes.
