...
- `ActionHttp.layer(App, { prefix?, openapiPath? })` registers `POST <prefix>/<name>` per HTTP-enabled action plus `GET /openapi.json` (default prefix `/api/actions`). It registers nothing for an MCP-only group.
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Input                       | Decoded with the full input codec; failure is an empty **400**                                              | Decoded with the full input codec; failure is `InvalidParams`, which this snapshot presents as an `isError` result with the message |
...
Object schemas keep Effect's default excess-field behaviour (stripped unless the schema rejects them). OpenAPI component names, references and operation IDs (`actions.<name>`) are generated by Effect. `HttpApiClient` runs the same codecs as the server: `client.actions.double({ payload: { value: 21 } })` encodes to the wire string and decodes the reply to `42`. Its **types** are loose, however: `ActionHttp.api` erases actions to `Action.Any`, so action names, payloads and results are not inferred per action (`unknown` result). Exact per-action client inference is a future refinement.

## What the spike proves

`vp test` exercises:

- Generated native HTTP RPC endpoints, configurable prefixes, MCP-only groups, and runtime codec integration with `HttpApiClient` (not exact per-action client inference).
...
- Schema codecs must not require services; handler dependencies can. Use JSON-compatible encoded representations for domain values.
