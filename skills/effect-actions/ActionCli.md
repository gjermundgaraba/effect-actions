...
/** The hook both projections accept. A CLI does not serialize failures, so it declares no surface errors. */
...
- The CLI declares no surface `errors`: it does not serialize failures, so a refusal is simply a typed failure of the command effect. Success values are still validated and encoded for output. The native parser also decodes `--input` and `--input-file` before the command runs, so unlike HTTP the hook runs after input decoding.
