...
- `implement` takes a complete record or an Effect producing one. Every action must have a handler; missing keys are compile errors. A record that still lacks a function for one at runtime (plain JavaScript, a cast: a missing key, `undefined`, any other non-function) throws at `implement`; a builder's record fails the adapter layer's build. Nothing is served with a handler missing. Handlers are read as plain properties, so an inherited one counts: a class instance's methods are handlers. They are called with the record as `this`, so a method may use it. What every object inherits from `Object.prototype` (`toString`, `constructor`, ...) is not a handler, so an action of that name still needs its own. Additional keys are not dispatched. It takes nothing else.
...
- `Missing handlers for group "<group>": <actions>` thrown at `implement`, or failing the layer build: the handler record has no function for those actions. Add them.
